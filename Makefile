...
build-debug build-release: build-%: build_%/CMakeCache.txt
	cmake --build build_$* -j $(NPROCS) --target sessions_management_service
...
service-start-debug service-start-release: service-start-%:
	cmake --build build_$* -v --target=start-sessions_management_service
...
install-debug install-release: install-%: build-%
	cmake --install build_$* -v --component sessions_management_service
...
docker-start-service-debug docker-start-service-release: docker-start-service-%:
	$(DOCKER_COMPOSE) run -p 8080:8080 -p 8081:8081 --rm sessions_management_service-container make -- --in-docker-start-$*
...
docker-cmake-debug docker-build-debug docker-test-debug docker-clean-debug docker-install-debug docker-cmake-release docker-build-release docker-test-release docker-clean-release docker-install-release: docker-%:
	$(DOCKER_COMPOSE) run --rm sessions_management_service-container make $*
